...
(function() {
	...
	var $WebUploadMult = function(conf) {
		this.pictureId = conf.pictureId;
		this.uploadBtnId = conf.pictureId + "BtnId";
		this.uploadPreId = conf.pictureId + "PreId";
		this.uploadUrl = Feng.ctxPath + conf.uploadUrl;
		this.fileSizeLimit = 100 * 1024 * 1024;
		this.picWidth = 800;
		this.picHeight = 800;
        this.uploadBarId = null;
        this.multiple = conf.multiple || true;
	};
	...
	$WebUploadMult.prototype = {
		...
		bindEvent : function(bindedObj) {
			var me =  this;
			bindedObj.on('filesQueued', function(files) {
				$.each(files, function(i, item){
					var $li = $('<div><img width="100px" height="100px"></div>');
					var $img = $li.find('img');
					$("#" + me.uploadPreId).append($li);
					// 生成缩略图
					bindedObj.makeThumb(item, function(error, src) {
						if (error) {
